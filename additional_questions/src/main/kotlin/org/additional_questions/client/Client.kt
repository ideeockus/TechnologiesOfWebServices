package org.additional_questions.client
...
import org.additional_questions.BookLib
import java.util.*
...
class BookServiceClientCLI(
    ...
    suspend fun getAllBooks(): List<BookLib> {
        ...
        val books: List<BookLib> = client.get("http://localhost:8080/books") {
    ...
    private suspend fun getBook(id: Int): BookLib {
        val authString = Base64.getEncoder().encodeToString("admin:admin".toByteArray())
        val book: BookLib = client.get("http://localhost:8080/books/$id") {
            header("Authorization", "Basic $authString")
        }.body()
        return book
    }
